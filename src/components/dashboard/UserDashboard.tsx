        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={async () => {
                  alert("Starting emergency sync of " + partners.length + " partners. Please do not close the page...");
                  try {
                    ...
                    const { savePartner } = await import('../../lib/userProfileService');
                    ...
                    for (const p of partners) {
                      ...
                      if (!isUUID(partnerToSave.id)) {
                        partnerToSave.id = uuidv4();
                      }
                    ...
                    if (errors.length > 0) {
                      alert(`Synced ${successCount}/${partners.length} partners.\n\nErrors:\n${errors.join('\n')}`);
                    } else {
                      // Trigger a store update if IDs were fixed
                      loadFromCloud();
                      alert(`Success! All ${partners.length} partners synced to cloud. Pull down to refresh on mobile.`);
                    }
                  } catch (err: any) {
                    alert("Critical Error: " + err.message);
                  }
                }}
                className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors text-sm font-medium mr-2"
              >
                Emergency Sync Data
              </button>
              <button
