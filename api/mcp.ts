...
// DO NOT import api/v1/_*.ts from this file. api/v1 is CommonJS (so Vercel
// inlines its transitive deps like src/lib/astro into the v1 bundle) while
// this function is ESM — cross-importing produces a bundle that crashes on
// cold start with "Cannot use import statement outside a module"
// (FUNCTION_INVOCATION_FAILED). Tool calls go to /api/v1/* over HTTP instead.
...
// HTTP fetch sidesteps the ESM/CJS boundary (see V1_ENDPOINTS note above)
// AND lets each function use its own deployment bundle.
