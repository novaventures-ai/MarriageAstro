...
const OUTPUT_JSON = path.resolve('C:/Users/Rahul.Govalkar/Documents/Personal/Scraper ai/ultra_deep_new_filter.json');
...
async function ultraDeepAnalyze() {
    ...
    // Step 1: Read the scores CSV to find all 'good' verdict names
    const goodNames: Set<string> = new Set();
    const scoresStream = fs.createReadStream(SCORES_CSV, { encoding: 'utf-8' });
    for await (const sRow of scoresStream.pipe(csv({ mapHeaders: ({ header }: { header: string }) => header.replace(/^\uFEFF/, '').trim() }))) {
        const verdict = ((sRow as any)['Verdict'] || '').trim().toLowerCase();
        const name = ((sRow as any)['Name'] || '').trim();
        if (verdict === 'good' && name) {
            goodNames.add(name.toLowerCase());
        }
    }
    console.log(`Found ${goodNames.size} profiles with 'good' verdict.\n`);
    ...
    // Filter to only good-verdict profiles
    const topRows = rows;
        const name = (row['Name'] || '').trim().toLowerCase();
        return goodNames.has(name);
    });
